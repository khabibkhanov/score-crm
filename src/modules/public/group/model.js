...
const groups = async ({ studentId = 0, assistantId = 0 }, { groups: userGroups, role }) => {
	...
	let table = {
		...
		'teacher': groups.map( group => ({ type: 'link', text: group.teacher_full_name, link: '/students?teacherId=' + group.teacher_id }) ),
	...
	if(role == 'student') {
		table['teacher'] = groups.map( group => ({ type: 'text', text: group.teacher_full_name }) )
	}
